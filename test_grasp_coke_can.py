...
def save_obs_images(obs, step):
    # Display initial observation images
    print("\n📸 Camera observations:")
    overhead_rgb = obs["sensor_data"]["overhead_camera"]["rgb"]
...
print(f"  Object: {env.unwrapped.obj.name}")
...
if save_image:
    save_obs_images(obs, i)
while True:
    action = env.action_space.sample()
    obs, reward, terminated, truncated, info = env.step(action)
    success = info['success'][0].item() if hasattr(info['success'], 'item') else info['success']
    is_grasped = info['is_grasped'][0].item() if hasattr(info['is_grasped'], 'item') else info['is_grasped']
    print(f"  Step {i+1}: success={success}, is_grasped={is_grasped}")
    i+=1
