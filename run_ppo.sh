...
tmux new-session -d -s ppo1 -x 200 -y 50


tmux send-keys -t ppo1:0 "export CUDA_VISIBLE_DEVICES=$CUDA_DEVICE" Enter
tmux send-keys -t ppo1:0 "export WANDB_API_KEY=$MY_WANDB_API_KEY" Enter
...
CMD="conda activate mnsk && python examples/baselines/ppo/ppo_rgb.py \
  ...
  --eval_freq=10 \
  --num-steps=20 \
  --exp_name=\"PutSpoon-ppo_rgb-slower-orient\" \
  --wandb_entity=\"chad_qian_tamu\" \
  --wandb_project_name=\"ManiSkill-PPO\" \
  --wandb_group=\"PutSpoon-Experiments\" \
  --track"

tmux send-keys -t ppo1:0 "$CMD" Enter

echo "Command started in tmux session 'ppo1' window 0"
